...
mongoose.connect(process.env.DATABASE_URL).then(async () => {
  ...
  // Fetch only metadata + first 30 bytes of data field for fast analysis
  const allStoredMetadata = await storedFilesCol.find({}, { projection: { filename: 1, contentType: 1, size: 1, data: { $slice: 30 } } }).toArray();
  ...
  // Find a valid PDF template for repairs
        ...
        if (storedDoc) {
          ...
          if (buf && buf.length >= 4) {
            ...
            if (fileName.toLowerCase().endsWith('.pdf')) {
              binaryValid = strHead.includes('%PDF') && size > 500;
            } else {
